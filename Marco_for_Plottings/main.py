...
data = pd.read_csv(DATA_PATH, header=None, sep="\s+", index_col=False, skiprows=18).to_numpy()
#  remove time column
data = np.copy(data[:,1:])
...
for ii in range(len(FreQ)): 
    modelSetup1modes.append( Wmodel( nodes_model, connectivity_model, Setup1.get_mode_shape(method = 'efdd', num = ii+1 ) ) ) # num = 1,2,...

    kwargsWmodel =  {'color':'#e3e3e3', 'linestyle':'solid'}
    kwargsmarkers = {'color':'#e3e3e3', 'marker':'none'}
    kwargsmonnodes = {'color':'red', 'marker':'o'}
    kwargsannotations = {'color':'red', 'fontsize':10, 'xytext':(-3,-3), 'textcoords':'offset points', 'ha':'right','va':'bottom'}
    kwargsarrows =  {'color':'red', 'arrowstyle':"-|>" , 'lw':1, 'mutation_scale':SCFCARROW_LIST[ii]}
    figs3DSetup1.append( modelSetup1modes[-1].plot_mode_shape3D( SCFC_LIST[ii], kwargsWmodel, kwargsmarkers, kwargsmonnodes, kwargsannotations, kwargsarrows, save_to_file_path = OUTPATH ) )
    # ax = plt.gca()
    # ax.view_init(elev=70, azim=-125)
    # ax.set_box_aspect([2,2,1])
    # ax.dist=AX_DIST[ii]
    kwargsWmodel =  {'color':'#e3e3e3', 'linestyle':'solid', 'zorder':1}
    kwargsarrows2D =  {'color':'red', 'lw':1, 'zorder':2, 'head_length':0.8, 'head_width':0.8,}
    zlim = 6
    modelSetup1modes[-1].plot_2D_top_view(zlim, SCFC_LIST[ii], kwargsWmodel, kwargsmarkers, kwargsmonnodes, kwargsannotations, kwargsarrows2D, save_to_file_path = OUTPATH)
    zlim = 3
    modelSetup1modes[-1].plot_2D_top_view(zlim, SCFC_LIST[ii], kwargsWmodel, kwargsmarkers, kwargsmonnodes, kwargsannotations, kwargsarrows2D, save_to_file_path = OUTPATH)
