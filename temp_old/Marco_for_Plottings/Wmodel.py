...
class Wmodel:
    ...
    """
    def __init__(self, nodes_coord: np.ndarray , connectivity: np.ndarray, mode: list) -> None:
        """
        ----------
        Parameters
        ----------
    ...
    def plot_mode_shape3D(self, scfc : Union[int, float] , kwargsWmodel : dict, kwargsmarkers : dict, kwargsmonnodes : dict, kwargsannotations : dict, kwargsarrows : dict, save_to_file_path : str = ''):
        """
        ...
        """
        fig,ax = plot3Dframe(self.nodes_coord, self.connectivity, kwargs_plot_lines=kwargsWmodel, kwargs_plot_markers = kwargsmarkers)
        ...
        for ii,monitored_nodes in enumerate(self.mode_nodes):
            if monitored_nodes not in plotted_mon_nodes:
                ...
                ax.scatter(*self.nodes_coord[monitored_nodes==self.nodes_numbering,:][0], **kwargsmonnodes)
                annotate3d(ax, f'{monitored_nodes:.0f}', xyz=self.nodes_coord[monitored_nodes==self.nodes_numbering,:][0], **kwargsannotations )
        for ii,modal_component in enumerate(self.mode_shape):
            arrow3d(ax, *self._nodes[self.mode_nodes[ii]==self.nodes_numbering,1:][0], \
                    *(scfc * modal_component * self.mode_dofs[:,ii]) , **kwargsarrows)
        ax.grid(False);ax.xaxis.set_pane_color((1.0, 1.0, 1.0, 0.0)); ax.yaxis.set_pane_color((1.0, 1.0, 1.0, 0.0)); ax.zaxis.set_pane_color((1.0, 1.0, 1.0, 0.0))
        plt.title(f'Freq. {self.freq:.2f} Hz')
        if len(save_to_file_path):
            plt.savefig(save_to_file_path + os.sep + f'MODE_shape_3D_freq_{self.freq:.2f}Hz_{self.method}'+'.pdf')
            plt.savefig(save_to_file_path + os.sep + f'MODE_shape_3D_freq_{self.freq:2f}Hz_{self.method}'+'.png')
            pickle.dump(fig, open(save_to_file_path + os.sep + f'MODE_shape_3D_freq_{self.freq:.2f}Hz_{self.method}.pickle', 'wb') )
        return fig

    def plot_2D_top_view(self, zlevel : float, scfc : Union[int, float] , kwargsWmodel : dict, kwargsmarkers : dict, kwargsmonnodes : dict, kwargsannotations : dict, kwargsarrows : dict, save_to_file_path : str = '', zspan = 0.05):
        """
        ...
        mask = ( (self.nodes_coord[:,2] < zlim[0]).astype(int) * (self.nodes_coord[:,2] > zlim[1]).astype(int) ).astype(dtype=bool)
        sel_nodes = self.nodes_numbering[mask]
        # sel_coord = self.nodes_coord[mask,:]

        mask = (np.in1d(self.connectivity[:,0], sel_nodes).astype(int) * np.in1d(self.connectivity[:,1], sel_nodes).astype(int)).astype(dtype=bool)
        ...
        fig,ax = plot2Dframe(self.nodes_coord[:,:-1], sel_connectivity, kwargs_plot_lines=kwargsWmodel, kwargs_plot_markers = kwargsmarkers)
        ...
        for ii,monitored_nodes in enumerate(self.mode_nodes):
            if monitored_nodes not in plotted_mon_nodes and sel_mode_nodes[ii]:
                plotted_mon_nodes.append(monitored_nodes)
                ax.scatter(*self.nodes_coord[monitored_nodes==self.nodes_numbering,:][0], **kwargsmonnodes)
                ax.annotate(f'{monitored_nodes:.0f}', xy=self.nodes_coord[monitored_nodes==self.nodes_numbering,:-1][0], **kwargsannotations )
        for ii,modal_component in enumerate(self.mode_shape):
            if sel_mode_nodes[ii] and not np.array_equal( self.mode_dofs[:,ii] , np.array([0.,0.,1.]) ):
                ...
                # ax.add_patch(arrow)
                arrow = mpatches.FancyArrow(self.nodes_coord[self.mode_nodes[ii]==self.nodes_numbering,0][0], self.nodes_coord[self.mode_nodes[ii]==self.nodes_numbering,1][0], \
                                            (scfc * modal_component * self.mode_dofs[:,ii])[0], (scfc * modal_component * self.mode_dofs[:,ii])[1], \
        ...
        plt.tight_layout()
        if len(save_to_file_path):
            plt.savefig(save_to_file_path + os.sep + f'MODE_shape_2D_freq_{self.freq:.2f}Hz_view_from_z_{zlim[0]:.2f}m_{self.method}'+'.pdf')
            plt.savefig(save_to_file_path + os.sep + f'MODE_shape_2D_freq_{self.freq:.2f}Hz_view_from_z_{zlim[0]:.2f}m_{self.method}'+'.png')
            pickle.dump(fig, open(save_to_file_path + os.sep + f'MODE_shape_2D_freq_{self.freq:.2f}Hz_view_from_z_{zlim[0]:.2f}m_{self.method}.pickle', 'wb') )
        return fig
