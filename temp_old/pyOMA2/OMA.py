...
from . import Plot_Modes, Sel_from_plot, tools
...
class Model:
    ...
    def FDDmodEX(self, ndf=5):
        ...
        for freq in sel_freq:
            ...
            idxlim = (
                np.argmin(np.abs(freqs - lim[0])),
                np.argmin(np.abs(freqs - lim[1])),
            )  # Indices of the limits

            # Ratios between the first and second singular value
            diffS1S2 = (
                S_val[0, 0, idxlim[0] : idxlim[1]] / S_val[1, 1, idxlim[0] : idxlim[1]]
            )
            ...
            fi_FDDn = (
                fi_FDD / fi_FDD[np.argmax(np.abs(fi_FDD))]
    ...
    def EFDDmodEX(
        self, ndf=5, cm=1, MAClim=0.85, sppk=3, npmax=30, method="FSDD", plot=False
    ):
        """
        Bla bla bla
        """
        ...
        for n in range(len(sel_freq)):  # looping through all frequencies to estimate
                ...
                if method == "FSDD":
                    ...
                    SDOFbell += np.array(
                        [
                            _fi.conj().T
                            @ PSD_matr[:, :, _l]
                            @ _fi  # Enhanced PSD matrix (frequency filtered)
            ...
            FIs = [SDOFbell[idSV[u]] * SDOFms[idSV[u], :] for u in range(len(idSV))]
            ...
            sgn1 = np.diff(
                sgn, axis=0
            )  # finding where the sign changes (intersept with x=0)
            ...
            maxSDOFcorr_idx = [
                np.argmin(abs(normSDOFcorr - maxx)) for maxx in maxSDOFcorr
            ]
            minSDOFcorr_idx = [
                np.argmin(abs(normSDOFcorr - minn)) for minn in minSDOFcorr
            ]
            ...
            minmax_fit_idx = np.array(
                [minmax_idx[_a] for _a in range(sppk, sppk + npmax)]
            )

            # estimating the natural frequency from the distance between the peaks
            Td = (
                np.diff(timeLag[minmax_fit_idx]) * 2
            ...
            if plot:
                ...
                _ax1.plot(
                    fsval, 10 * np.log10(SDOFbell[idSV].real), c="r", label="SDOF bell"
                )
                ...
                _ax3.plot(
                    timeLag[: minmax_fit_idx[-1]], normSDOFcorr[: minmax_fit_idx[-1]]
                )
    ...
    def SSIcov(self, br, ordmin=0, ordmax=None, method="1"):
        ...
        R_is = np.array(
            [
                1 / (self.Ndat - _s) * (Yy[:, : self.Ndat - _s] @ Yy[:, _s:].T)
        ...
        for ii in range(self.SSI_ordmin, self.SSI_ordmax + 1, 2):
            ...
            Mcomp = np.array(
                [
                    Mcomp[:, ii] / Mcomp[np.argmax(abs(Mcomp[:, ii])), ii]
                    for ii in range(Mcomp.shape[1])
                ]
            ).reshape(-1, Nch)
    ...
    def SSIdat(self, br, ordmin=0, ordmax=None, method="1"):
        ...
        H = np.array([(1 / j**0.5) * Yy[:, k : k + j] for k in range(2 * br)]).reshape(
            Nch * 2 * br, j
        )
        ...
        for ii in range(self.SSI_ordmin, self.SSI_ordmax + 1, 2):
            ...
            if method == "1":  # Method 1
                ...
                Sp1 = (
                    np.linalg.pinv(O[: O.shape[0] - Nch, :]) @ P_im1
                )  # kalman state sequence S_(i+1)
    ...
    def pLSCF(self, ordmax, df=0.01, pov=0.5, window="hann"):
        ...
        for j in range(1, ordmax + 1):  # loop for increasing model order
            ...
            alfa = np.linalg.solve(
                -M[: j * Nch, : j * Nch], M[: j * Nch, j * Nch : (j + 1) * Nch]
            )  # 4.169
            ...
            Ls[: j * Nch, j - 1] = lambd
            Fr[: j * Nch, j - 1] = abs(lambd) / (
                2 * np.pi
            )  # Natural frequencies (Hz) 4.137
            Sm[: j * Nch, j - 1] = (np.real(lambd)) / abs(
                lambd
    ...
    def pLSCFmodEx(self):
        ...
        for ww in w_sel:  # loop su poli selezionati
            ...
            for kk in range(idx_w - 20, idx_w + 20):
                GL += np.array(
                    [
                        self.Sy[kk, :, :] / (1j * self.ws[kk] - sel_lam[jj])
                        for jj in range(Nm)
                    ]
                ).reshape(-1, Nch)
                    ...
                    [
                        np.array(
                            [
                                np.eye(Nch)
                                / (
                                    (1j * self.ws[kk] - sel_lam[jj1])
                                    * (1j * self.ws[kk] - sel_lam[jj2])
                                )
                                for jj2 in range(Nm)
                            ]
                        )
    ...
    def def_geo(self, nodes_coord, directions, mapping=None):
        ...
        try:
            ...
            raise Exception(
                "The number of nodes and directions must equal the number of channels"
            )
