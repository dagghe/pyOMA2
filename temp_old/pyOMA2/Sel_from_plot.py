...
class SelFromPlot:
    def __init__(self, Model, plot, freqlim=None):
        ...
        if plot == "SSI" or plot == "pLSCF":
            self.fig.canvas.mpl_connect(
                "button_press_event", lambda x: self.on_click_SSI(x, plot)
            )
    ...
    def plot_svPSD(self, update_ticks=False):
        ...
        if not update_ticks:
            ...
            self.ax2.set_title(f"Singular values plot - (Freq. res. ={df})")
    ...
    def get_stab(self, plot, err_fn=0.01, err_xi=0.05, err_ms=0.02):
        ...
        if plot == "SSI":
            ...
            self.Lab = tools._stab_SSI(
                Fr, Sm, Ms, ordmin, ordmax, err_fn=err_fn, err_xi=err_xi, err_ms=err_ms
            )
            ...
            nch = self.Model.Nch
            self.Lab = tools._stab_pLSCF(
                Fr, Sm, ordmax, err_fn=err_fn, err_xi=err_xi, nch=nch
            )
        ...
        if not update_ticks:
            ...
            if plot == "SSI":
                ...
                if self.hide_poles:
                    ...
                    self.ax1.scatter(
                        x4, y, marker="o", s=4, c="#FFA500", label="Stable damping"
                    )
                    self.ax1.scatter(
                        x5, y, marker="o", s=4, c="#FFA500", label="Stable mode shape"
                    )
                    self.ax1.scatter(
                        x6, y, marker="o", s=4, c="#FFA500", label="Stable frequency"
                    )
                    self.ax1.scatter(x7, y, marker="o", s=4, c="r", label="Unstable pole")
            ...
            # -----------------------
                ...
                if self.hide_poles:
                    ...
                    self.ax1.scatter(
                        x1, y, marker="o", s=4, c="#FFFF00", label="Stable damping"
                    )
                    self.ax1.scatter(
                        x2, y, marker="o", s=4, c="#FFFF00", label="Stable frequency"
                    )
                    self.ax1.scatter(x3, y, marker="o", s=4, c="r", label="Unstable pole")
    ...
    def get_closest_pole(self, plot):
        ...
        y_ind = int(
            np.argmin(np.abs(np.arange(Fr.shape[1]) - self.y_data_pole))
