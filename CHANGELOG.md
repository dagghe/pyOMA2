...
- `gen.match_modes` pairs the modes of different setups by frequency and reference-sensor
  MAC; `gen.merge_modal_params` merges a modal parameter over the setups that identified
  each mode, inverse-variance weighted when uncertainties are available.
...
- PoSER merge ignored the SSI uncertainties (`calc_unc=True`); frequencies and damping
  ratios are now inverse-variance weighted when every contributing setup provides them.
...
- Replaced PDM with uv as package manager
...
- Loop index in pyoma2.functions.gen.SC_apply()


## [1.2.0] - 2025-09-09

This release includes all changes from beta releases [1.2.0b1](#120b1---2025-02-12), [1.2.0b2](#120b2---2025-02-13), [1.2.0b3](#120b3---2025-06-11), and [1.2.0b4](#120b4---2025-06-18).
...
- HC_freqlim() and synt_spectr() functions to ssi.py functions file
...
- bug in sel_from_plot.py where the step run_parameter for ssi was not considered
