...
class AniMode:
    ...
    def plot3D(self):
        nr_iter = 200
        Data = np.repeat(self.nodes_coord[:, :, np.newaxis], nr_iter, axis=2).astype(
            float
        )
        ...
        # Initialize scatters
        scatters = [
            self.ax.scatter(Data[i, 0:1, 0], Data[i, 1:2, 0], Data[i, 2:, 0])
            for i in range(Data.shape[0])
        ]
    ...
    def plot2D(self):
        ...
        # Initialize scatters
        scatters = [
            self.ax.scatter(Data[i, 0, 0], Data[i, 1, 0]) for i in range(Data.shape[0])
        ]
