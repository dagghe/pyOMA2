...
class MonSetup:
    ...
    def __init__(self, data, setup_layout: list, fs: float) -> None:
        ...
        if isinstance(data, str):
            try:
                ...
                raise TypeError(
                    "Data argument type not recognized. Please provide data path as string to data file to be imported (Supported file format .txt, .csv, .pkl), or directly the numpy data array."
                )
        else:
            try:
                self.data = data
        ...
        for ii, tmp in enumerate(setup_layout):
            ...
            if dof == "x":
                ...
                raise TypeError(
                    "Unrecognized setup_layout. Please provide setup_layout as a list of string of the form NodeNumber-MonitoredDOF, e.g. ['1x','1y','1z'] means a triaxial sensor placed in node 1."
                )
    ...
    def decimate(self, q: int):
        ...
        if q < 1:  # q : decimation factor
            print(
                "If you want to decimate data, please provide a decimation factor greater than 1."
            )
        else:
            try:
                ...
                raise ValueError(
                    f"Error raised with this decimation factor {q}. Please, provide an integer factor greater than 1 if you want to decimate."
                )

    def plot_signals(
        self,
        save_to_file_path: str = "",
        axis_labels: list = ["Time [s]", "Amplitude [m/s]"],
        **kwargs,
    ):
        """
        ...
        for ii, dof in enumerate(["x", "y", "z"]):
            if any(self.mon_nodes_dofs[ii + 1, :]):
                    ...
                    if self.mon_nodes_dofs[ii + 1, jj] == 1:
                        custom_legend_elements.append(
                            Line2D(
                                ...
                                [0],
                                color=palette[jj],
                                label=f"Sensor {self.mon_nodes_dofs[0,jj]:.0f} {dof}",
                                **kwargs,
                            )
                        )
                        ax.plot(
                            np.linspace(
                                0, self.data.shape[0] / self.fs, self.data.shape[0]
                            ),
                            self.data[:, jj],
                            color=palette[jj],
                            **kwargs,
                ...
                if len(save_to_file_path):
                    plt.savefig(
                        save_to_file_path + os.sep + f"Monitored_signals_{dof}" + ".pdf"
                    )
                    plt.savefig(
                        save_to_file_path + os.sep + f"Monitored_signals_{dof}" + ".png"
                    )
        return figures
        ...
        **kwargs : dictionary
        ...
        if len(save_to_file_path):
            ...
            pickle.dump(
                fig,
                open(
                    save_to_file_path + os.sep + f"SVD_PSD_decimation_{self.q:d}.pickle",
                    "wb",
                ),
            )
            fig.savefig(
                save_to_file_path + os.sep + f"SVD_PSD_decimation_{self.q:d}" + ".pdf"
            )
            fig.savefig(
                save_to_file_path + os.sep + f"SVD_PSD_decimation_{self.q:d}" + ".png"
            )
            with open(
                save_to_file_path + os.sep + f"FDD_decimation_{self.q:d}.pkl", "wb"
            ) as file:
        ...
        if len(save_to_file_path):
            ...
            pickle.dump(
                ...
                open(
                    save_to_file_path
                    + os.sep
                    + f"StabDiag_decimation_{self.q:d}_br_{self.br:d}.pickle",
                    "wb",
                ),
            )
            fig.savefig(
                save_to_file_path
                + os.sep
                + f"StabDiag_decimation_{self.q:d}_br_{self.br:d}"
                + ".pdf"
            )
            fig.savefig(
                save_to_file_path
            ...
            with open(
                save_to_file_path
                + os.sep
                + f"SSIcov_decimation_{self.q:d}_br_{self.br:d}.pkl",
                "wb",
            ) as file:
                pickle.dump(self.ssicov, file)
        return fig

    def overlap_stabdiag_svpsd(
        self, save_to_file_path: str = "", selection_criteria: int = 4, num_sv: int = 3
    ):
        """
        ...
        ax1 = sns.scatterplot(
            x=Selected_poles["Frequency"],
            y=Selected_poles["Order"] * 2,
            hue=Selected_poles["Label"],
            palette=_colors,
        )
        ...
        for _i in range(num_sv):
            ...
            ax2.plot(
                self.fdd["f"][:],
                10 * np.log10(self.fdd["Singular Values"][_i, _i]),
                zorder=1,
                color=palette[_i],
            )  # decibel
        ...
        if len(save_to_file_path):
            plt.savefig(
                save_to_file_path
                + os.sep
                + f"StabDiag_overlap_SVD_decimation_{self.q:d}_BR_{self.br:d}_selection_{selection_criteria:d}"
                + ".pdf"
            )
            plt.savefig(
                save_to_file_path
            ...
            pickle.dump(
                ...
                open(
                    save_to_file_path
                    + os.sep
                    + f"StabDiag_overlap_SVD_decimation_{self.q:d}_BR_{self.br:d}_selection_{selection_criteria:d}.pickle",
                    "wb",
                ),
            )
        return fig1
    ...
    def get_modal_prop(
        self, method: str, FreQ: list, save_to_file_path: str = "", **kwargs
    ):
        """
        ...
        if method.upper() == "EFDD" or method.upper() == "FSDD":
            try:
                if hasattr(self, "fdd"):
                    ...
                    self.res_efdd = oma.EFDDmodEX(
                        FreQ, self.fdd, method=method.upper(), **kwargs
                    )
                    ...
                    self.mac_efdd = np.reshape(
                        [
                            oma.MaC(self.ms_efdd[:, l], self.ms_efdd[:, k]).real
                            for k in range(self._n_modes_efdd)
                            for l in range(self._n_modes_efdd)
                        ],  # (self._n_modes_efdd*self._n_modes_efdd) list of MAC values
                        (self._n_modes_efdd, self._n_modes_efdd),
                    )  # new (real) shape (self._n_modes_efdd x self._n_modes_efdd) of the MAC matrix
                    MACnames = [f"{ii[0]:.2f} Hz" for ii in self.res_efdd["Frequencies"]]
                    self.mac_efdd = pd.DataFrame(
                        self.mac_efdd, columns=MACnames, index=MACnames
                    )
                    fig, ax = plt.subplots()
                    sns.heatmap(
                        self.mac_efdd,
                    ...
                    if len(save_to_file_path):
                        with open(
                            save_to_file_path
                            + os.sep
                            + f"Res_EFDD_decimation_{self.q:d}.pkl",
                            "wb",
                        ) as file:
                        ...
                        np.savetxt(
                            save_to_file_path
                            + os.sep
                            + f"Res_EFDD_decimation_{self.q:d}_FREQ.txt",
                            self.res_efdd["Frequencies"],
                            fmt="%.6f",
                        )
                        np.savetxt(
                            save_to_file_path
                            + os.sep
                            + f"Res_EFDD_decimation_{self.q:d}_DAMPING.txt",
                            self.res_efdd["Damping"],
                            fmt="%.6f",
                        )
                        np.savetxt(
                            save_to_file_path
                            + os.sep
                            + f"Res_EFDD_decimation_{self.q:d}_MODESHAPES_complex.txt",
                            self.res_efdd["Mode Shapes"],
                            fmt="%.6f",
                        )
                        np.savetxt(
                            save_to_file_path
                            + os.sep
                            + f"Res_EFDD_decimation_{self.q:d}_MODESHAPES_real.txt",
                            self.ms_efdd,
                            fmt="%.6f",
                        )
                        ...
                        [
                            list_of_plots.remove(item)
                            for item in num_plots_so_Far
                            if item in list_of_plots
                        ]

                        for ii, _i in enumerate(list_of_plots):
                            if _i == list_of_plots[-1]:
                                plt.figure(_i).savefig(
                                    save_to_file_path
                                    + os.sep
                                    + f"MAC_EFDD_decimation_{self.q:d}.pdf"
                                )
                                plt.figure(_i).savefig(
                                    save_to_file_path
                                    + os.sep
                                    + f"MAC_EFDD_decimation_{self.q:d}.png"
                                )
                            else:
                                plt.figure(_i).savefig(
                                    save_to_file_path
                                    + os.sep
                                    + f'EFDD_SDOF_bell_{self.res_efdd["Frequencies"][ii][0]:.1f}.pdf'
                                )
                                plt.figure(_i).savefig(
                                    save_to_file_path
                                    + os.sep
                                    + f'EFDD_SDOF_bell_{self.res_efdd["Frequencies"][ii][0]:.1f}.png'
                                )
                    else:
                        raise NameError(
                            f"The attribute fdd does not exist yet! Please run the method svd_psd() prior to get modal results with method {method.upper()}."
                        )
            ...
            try:
                if hasattr(self, "ssicov"):
                    self.res_ssicov = oma.SSIModEX(
                        FreQ, self.ssicov, **kwargs
                    ...
                    self.mac_ssicov = np.reshape(
                        [
                            oma.MaC(self.ms_ssicov[:, l], self.ms_ssicov[:, k]).real
                            for k in range(self._n_modes_ssicov)
                            for l in range(self._n_modes_ssicov)
                        ],  # (self._n_modes_efdd*self._n_modes_efdd) list of MAC values
                        (self._n_modes_ssicov, self._n_modes_ssicov),
                    )  # new (real) shape (self._n_modes_efdd x self._n_modes_efdd) of the MAC matrix
                    MACnames = [f"{ii:.2f} Hz" for ii in self.res_ssicov["Frequencies"]]
                    self.mac_ssicov = pd.DataFrame(
                        self.mac_ssicov, columns=MACnames, index=MACnames
                    )
                    fig, ax = plt.subplots()
                    sns.heatmap(
                        self.mac_ssicov,
                    ...
                    if len(save_to_file_path):
                        np.savetxt(
                            save_to_file_path
                            + os.sep
                            + f"Res_SSIcov_decimation_{self.q:d}_BR_{self.br:d}_FREQ.txt",
                            self.res_ssicov["Frequencies"],
                            fmt="%.6f",
                        )
                        np.savetxt(
                            save_to_file_path
                            + os.sep
                            + f"Res_SSIcov_decimation_{self.q:d}_BR_{self.br:d}_DAMPING.txt",
                            self.res_ssicov["Damping"],
                            fmt="%.6f",
                        )
                        np.savetxt(
                            save_to_file_path
                            + os.sep
                            + f"Res_SSIcov_decimation_{self.q:d}_BR_{self.br:d}_MODESHAPES_complex.txt",
                            self.res_ssicov["Mode Shapes"],
                            fmt="%.6f",
                        )
                        np.savetxt(
                            save_to_file_path
                            + os.sep
                            + f"Res_SSIcov_decimation_{self.q:d}_BR_{self.br:d}_MODESHAPES_real.txt",
                            self.ms_ssicov,
                            fmt="%.6f",
                        )
                        ...
                        plt.savefig(
                            save_to_file_path
                            + os.sep
                            + f"MAC_SSIcov_decimation_{self.q:d}_BR_{self.br:d}"
                            + ".pdf"
                        )
                        plt.savefig(
                            save_to_file_path
                        ...
                        raise NameError(
                            f"The attribute ssicov does not exist yet! Please run the method runssicov() prior to get modal results with method {method.upper()}."
                        )
    ...
    def crossmac_ssi_fdd(
        ...
        self.crossmac_efdd_vs_ssi = np.reshape(
            [
                oma.MaC(self.ms_efdd[:, l], self.ms_ssicov[:, k]).real
                for k in range(_n_modes)
                for l in range(_n_modes)
            ],  # (self._n_modes_efdd*self._n_modes_efdd) list of MAC values
            (_n_modes, _n_modes),
        )  # new (real) shape (self._n_modes_efdd x self._n_modes_efdd) of the MAC matrix
        MACnames = [f"{ii:.2f} Hz" for ii in self.res_ssicov["Frequencies"]]
        self.crossmac_efdd_vs_ssi = pd.DataFrame(
            self.crossmac_efdd_vs_ssi, columns=MACnames, index=MACnames
        )
        fig, ax = plt.subplots()
        sns.heatmap(
            self.crossmac_efdd_vs_ssi,
            cmap="jet",
            ax=ax,
            annot=True,
            fmt=".3f",
    ...
    def get_mode_shape(
        self, method: str, num: int = 1
    ):  # mode_num is numbered from 1 for the user
        """
        ...
        if method.upper() == "EFDD" or method.upper() == "FSDD":
            try:
                ...
                print(
                    ...
                    "Possible cause: non-valid mode number.",
                )
                raise
        elif method.upper() == "SSICOV":
            try:
