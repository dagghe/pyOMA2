...
def merge_modal_params(
    values: np.ndarray, stds: typing.Optional[np.ndarray] = None
) -> typing.Tuple[np.ndarray, np.ndarray]:
    """
    Merges a modal parameter (e.g. natural frequencies) identified in several setups.

    Parameters
    ----------
    values : np.ndarray
        Values identified in each setup, of shape (n_setups, n_modes), NaN where a setup
        did not identify the mode.
    stds : np.ndarray, optional
        Standard deviations of `values` (e.g. from SSI with ``calc_unc=True``), of shape
        (n_setups, n_modes). Default is None.

    Returns
    -------
    Tuple[np.ndarray, np.ndarray]
        Merged value and standard deviation of each mode, each of shape (n_modes,).

    Notes
    -----
    When every setup that identified a mode provides a positive, finite standard deviation,
    the mode is merged with the inverse-variance weighted mean, and its standard deviation
    is the one of the weighted mean, ``1 / sqrt(sum(1 / stds**2))``. Otherwise the mean and
    the standard deviation between the setups are returned.
    """
    values = np.asarray(values, dtype=float)
    stds = np.full(values.shape, np.nan) if stds is None else np.asarray(stds, float)
    mean = np.empty(values.shape[1])
    std = np.empty(values.shape[1])
    for k in range(values.shape[1]):
        found = ~np.isnan(values[:, k])
        val_k, std_k = values[found, k], stds[found, k]
        if np.all(np.isfinite(std_k) & (std_k > 0)):
            weights = 1 / std_k**2
            mean[k] = np.sum(weights * val_k) / np.sum(weights)
            std[k] = 1 / np.sqrt(np.sum(weights))
        else:
            mean[k] = np.mean(val_k)
            std[k] = np.std(val_k)
    return mean, std
