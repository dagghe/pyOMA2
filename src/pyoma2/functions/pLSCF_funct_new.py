...
# from pyoma2.functions import FDD_funct as FF
# from pyoma2.functions import Gen_funct as GF
# from pyoma2.functions import plot_funct as PF
...
def pLSCF(
    ...
    for n in range(1, ordmax + 1):
        ...
        Xoh = Xo.conj().T  # Calculate complex transpose
        ...
        for o in range(0, Nref):  # loop on channels
            ...
            M += 2 * (To - np.dot(np.dot(So.T, np.linalg.inv(Ro)), So))
...
def pLSCF_MPE(sel_freq, Fn_pol, Sm_pol, Ms_pol, order, Lab=None, deltaf=0.05, rtol=1e-2):
    """
...
#     data = np.load(
#         r"C:\Users\dpa\OneDrive - Norsk Treteknisk Institutt\Dokumenter\Dev\pyOMA2\src\pyoma2\test_data\palisaden\Palisaden_dataset.npy",  # noqa: E501
#         allow_pickle=True,
#     )
...
#     freq, PSD = FF.SD_Est(data.T, data.T, dt, nxseg=nxseg, method=methodSy)

#     ordmax = 50
#     _starttime = timeit.default_timer()
#     Ad, Bn = pLSCF(PSD, dt, ordmax, sgn_basf=sgn_basf)
#     Fns, Xis, Phis = pLSCF_Poles(Ad, Bn, dt, nxseg=nxseg, methodSy=methodSy)
#     Lab = GF.Lab_stab(
#         Fns, Xis, Phis, 0, ordmax, 1, err_fn=0.01, err_xi=0.05, err_ms=0.03, max_xi=0.1
#     )

#     fig, ax = PF.Stab_SSI_plot(Fns, Lab, 1, ordmax, freqlim=(1, 4), hide_poles=False)
#     sel_freq = [1.88, 2.42, 2.68]

#     # fn, xi, phi = pLSCF_MPE(sel_freq, PSD, Fns, Xis, Ls=None, order=40, dt=dt)
#     fn, xi, phi, orderout = pLSCF_MPE(
#         sel_freq, Fns, Xis, Phis, "find_min", Lab=Lab, deltaf=0.05, rtol=1e-2
#     )

#     print("The time difference is :", timeit.default_timer() - _starttime)
