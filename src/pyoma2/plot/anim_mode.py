...
def animate_scatters(iteration, Data, scatters, lines=None, sens_lines=None):
    ...
    if lines is not None and sens_lines is not None:
        for line, (idx1, idx2) in zip(lines, sens_lines):
            line.set_data(Data[[idx1, idx2], 0, iteration], Data[[idx1, idx2], 1, iteration])
...
# =============================================================================
    ...
    def plot3D(self):
        ...
        df_phi = pd.DataFrame({
            "sName":self.sens_names,
            "Phi":self.phi.real*self.scaleF
            },)
        ...
        newpoints1 = self.nodes_coord.add(df_phi_map*self.Geo.sens_sign*(-1), fill_value=0)
...
# """
        ...
        # Set ax options
        plot_funct.set_ax_options(self.ax,bg_color="w",
        ...
        if self.Geo.bg_nodes is not None:
            ...
            if self.Geo.bg_lines is not None:
                # if True plot
                plot_funct.plt_lines(self.ax,self.Geo.bg_nodes,self.Geo.bg_lines,
                      color="gray",alpha=0.5)
            if self.Geo.bg_surf is not None:
                # if True plot
                plot_funct.plt_surf(self.ax,self.Geo.bg_nodes,self.Geo.bg_surf
                         ,alpha=0.1)

        # Initialize scatters
        scatters = [self.ax.scatter(Data[i,0:1,0], Data[i,1:2,0], Data[i,2:,0],
                                    color="r") 
                    for i in range(Data.shape[0]) 
                    ]
        if self.Geo.sens_lines is not None:
            # Initialize lines based on sens_lines
            lines = [self.ax.add_line(Line3D([], [], []))
                     for _ in range(len(self.Geo.sens_lines))]
            # Creating the Animation object
            self.ani = animation.FuncAnimation(
                self.fig, animate_scatters, nr_iter, 
                fargs=(Data, scatters, lines, self.Geo.sens_lines),interval=20, 
                blit=False, repeat=True)

        else:
            self.ani = animation.FuncAnimation(
                self.fig, animate_scatters, nr_iter, fargs=(Data, scatters),
