...
def MIF_plot(sval, freq, freqlim=None, nSv="all"):
    ...
    for k in range(nSv):
        if k == 0:
            ax.plot(freq, 10 * np.log10(sval[k, k, :]/sval[k, k, :]
                    [np.argmax(sval[k, k, :])]), 'k', linewidth=2)
        else:
            ax.plot(
                freq, 10 * np.log10(sval[k, k, :]/sval[0, 0, :][np.argmax(sval[0, 0, :])]), 'grey')
...
def EFDD_FIT_plot(Fn,Xi,PerPlot, freqlim=None):
    ...
    for numb_mode in range(len(PerPlot)):
        ...
        ax1.plot(freq, 10*np.log10(Sval[0, 0] /
                 Sval[0, 0][np.argmax(Sval[0, 0])]), c='b')
        ax1.plot(fsval, 10*np.log10(SDOFbell[idSV].real/SDOFbell[np.argmax(SDOFbell)]), c='r',
                 label='SDOF bell')
        ...
        ax3.plot(time[:minmax_fit_idx[-1]],
                 normSDOFcorr[:minmax_fit_idx[-1]])
        ax3.scatter(time[minmax_fit_idx],
        ...
        ax4.plot(np.arange(len(minmax_fit_idx)),
                 lam/2*np.arange(len(minmax_fit_idx)))

        ax4.text(left, top, r'''$f_n$ = %.3f
...
def Stab_pLSCF_plot(Fn, Lab, ordmax, freqlim=None, hide_poles=True, Sval=None, nSv=None):
    ...
    if hide_poles:
        ...
        ax1.scatter(x1, y, marker='o', s=4, c='#FFFF00',
                    label="Stable damping")
        ax1.scatter(x2, y, marker='o', s=4, c='#FFFF00',
                    label="Stable frequency")
        ax1.scatter(x3, y, marker='o', s=4, c='r',
                    label="Unstable pole")
...
def Stab_SSI_plot(Fn, Lab, step, ordmax, ordmin=0, freqlim=None, hide_poles=True, Sval=None, nSv=None):
    # TO DO: Add sval plot on twin ax
    ...
    if hide_poles:
        ...
        ax1.scatter(x1, y1, marker='o', s=4, c='#FFFF00',
                    label="Stable frequency, stable mode shape")
        ax1.scatter(x2, y1, marker='o', s=4, c='#FFFF00',
                    label="Stable frequency, stable damping")
        ax1.scatter(x3, y1, marker='o', s=4, c='#FFFF00',
                    label="Stable damping, stable mode shape")
        ax1.scatter(x4, y1, marker='o', s=4, c='#FFA500',
                    label="Stable damping")
        ax1.scatter(x5, y1, marker='o', s=4, c='#FFA500',
                    label="Stable mode shape")
        ax1.scatter(x6, y1, marker='o', s=4, c='#FFA500',
                    label="Stable frequency")
        ax1.scatter(x7, y1, marker='o', s=4, c='r',
                    label="Unstable pole")
...
def Cluster_SSI_plot(Fn, Sm, Lab, ordmin=0, freqlim=None, hide_poles=True,):
    # Stable pole
    a = np.where(Lab == 7, Fn, np.nan)
    ...
    if hide_poles:
        ...
        ax1.scatter(x1, y1, marker='o', s=4, c='#FFFF00',
                    label="Stable frequency, stable mode shape")
        ax1.scatter(x2, y2, marker='o', s=4, c='#FFFF00',
                    label="Stable frequency, stable damping")
        ax1.scatter(x3, y3, marker='o', s=4, c='#FFFF00',
                    label="Stable damping, stable mode shape")
        ax1.scatter(x4, y4, marker='o', s=4, c='#FFA500',
                    label="Stable damping")
        ax1.scatter(x5, y5, marker='o', s=4, c='#FFA500',
                    label="Stable mode shape")
        ax1.scatter(x6, y6, marker='o', s=4, c='#FFA500',
                    label="Stable frequency")
        ax1.scatter(x7, y7, marker='o', s=4, c='r',
                    label="Unstable pole")
...
def Cluster_pLSCF_plot(Fn, Sm, Lab, ordmax, ordmin=0, freqlim=None, hide_poles=True,):
    # Stable pole
    a = np.where(Lab == 3, Fn, np.nan)
    ...
    if hide_poles:
        ...
        ax1.scatter(x2, y2, marker='o', s=4, c='#FFFF00',
                    label="Stable frequency")
        ax1.scatter(x3, y3, marker='o', s=4, c='r',
                    label="Unstable pole")
...
def plt_nodes(ax,nodes_coord,alpha=1,color="k"):
    ax.scatter(nodes_coord[:,0], nodes_coord[:,1], nodes_coord[:,2], alpha=alpha, color=color)
    return ax
...
def plt_lines(ax,nodes_coord,lines,alpha=1,color="k"):
    for ii in range(lines.shape[0]):
        ...
        ax.plot([StartX, EndX], 
                [StartY, EndY],
                [StartZ, EndZ], alpha=alpha, color=color)
    return ax
...
def plt_quiver(ax, nodes_coord, directions, scaleF=2, color="red",
               names=None, color_text="red", ):
    ...
    ax.quiver(xs0, ys0, zs0, (xs1-xs0), (ys1-ys0), (zs1-zs0), length=scaleF, color=color)
    if names is not None:
        ii=0
        for nam in names:
            ax.text(Points_f[ii,0], Points_f[ii,1], Points_f[ii,2], f"{nam}", color='red')
            ii += 1
    return ax


def set_ax_options(ax, bg_color ="w", remove_fill=True, remove_grid=True, remove_axis=True):
...
def set_view(ax, view):
    if view == "3D":
        ...
        elev = 0
        ax.view_init(azim=azim, elev=elev)
    else: raise ValueError(f"view must be one of (3D, xy, xz, yz), your input was '{view}'")
    return ax
...
def Transf_Matr(Nsens, mapping, s_names=None):
    ...
    if type(mapp[0]) is np.str_ and s_names is None:
        raise ValueError("If the mapping matrix elements are string, you need to specify the sensor's names")
...
# =============================================================================
def plt_data(data, dt, nc=1, names=None, unit="unit", 
             show_rms=False, len_Wrms=None):
    # show RMS of signal 
    if show_rms == True:
        # if the lenght of the window is not provided the RMS is calculated over the whole signal
        if len_Wrms==None:
            a_rmss = np.array([window_rms(data[:,_kk], data.shape[0]) for _kk in range(data.shape[1])])
        else:
            a_rmss = np.array([window_rms(data[:,_kk], len_Wrms) for _kk in range(data.shape[1])]).T
