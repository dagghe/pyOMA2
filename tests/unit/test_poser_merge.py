...
def test_aligned_modes_without_uncertainty_keep_index_merge() -> None:
    """Same modes in the same order, no uncertainty: identical to the index merge."""
...
def test_uncertainty_weights_the_merge() -> None:
    """A clean setup dominates a highly uncertain one (inverse-variance weighting)."""
    merged = _merge(
        SSIResult(
            Fn=np.array([10.00]),
            Fn_std=np.array([0.02]),
            Xi=np.array([0.010]),
            Xi_std=np.array([0.001]),
            Phi=_phi(REF_A + [0.5]),
        ),
        SSIResult(
            Fn=np.array([10.20]),
            Fn_std=np.array([0.50]),
            Xi=np.array([0.030]),
            Xi_std=np.array([0.010]),
            Phi=_phi(REF_A + [0.4]),
        ),
    )

    w_fn = 1 / np.array([0.02, 0.50]) ** 2
    w_xi = 1 / np.array([0.001, 0.010]) ** 2
    assert merged.Fn[0] < 10.10
    assert_allclose(merged.Fn, [np.sum(w_fn * [10.00, 10.20]) / np.sum(w_fn)])
    assert_allclose(merged.Fn_std, [1 / np.sqrt(np.sum(w_fn))])
    assert_allclose(merged.Xi, [np.sum(w_xi * [0.010, 0.030]) / np.sum(w_xi)])
    assert_allclose(merged.Xi_std, [1 / np.sqrt(np.sum(w_xi))])


def test_uncertainty_is_ignored_unless_every_contributor_provides_it() -> None:
    """Mixing weighted and unweighted setups falls back to the plain mean/std."""
    merged = _merge(
        SSIResult(
            Fn=np.array([10.00]),
            Fn_std=np.array([0.02]),
        ...
        SSIResult(Fn=np.array([10.20]), Xi=np.array([0.030]), Phi=_phi(REF_A + [0.4])),
    )

    assert_allclose(merged.Fn, [10.10])
    assert_allclose(merged.Fn_std, [0.10])
...
def test_missing_mode_and_uncertainty_are_merged_consistently() -> None:
    """Weights are computed over the contributing setups of each mode only."""
    merged = _merge(
        SSIResult(
            Fn=np.array([2.0, 5.0]),
            Fn_std=np.array([0.01, 0.05]),
            Xi=np.array([0.01, 0.02]),
            Xi_std=np.array([0.001, 0.002]),
            Phi=_phi(REF_A + [0.5], REF_B + [0.7]),
        ),
        SSIResult(
            Fn=np.array([2.15]),
            Fn_std=np.array([0.10]),
            Xi=np.array([0.01]),
            Xi_std=np.array([0.001]),
            Phi=_phi(REF_A + [0.4]),
        ),
    ...
    w0 = 1 / np.array([0.01, 0.10, 0.02]) ** 2
    assert merged.setups_used == [[0, 1, 2], [0, 2]]
    assert_allclose(merged.Fn[0], np.sum(w0 * [2.0, 2.15, 2.1]) / np.sum(w0))
    assert_allclose(merged.Fn[1], 5.1)
    assert_allclose(merged.Fn_std[1], 0.05 / np.sqrt(2))
